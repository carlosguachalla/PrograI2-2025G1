...
void agregarAlEjercito(const string& username, vector<int>& ejercito, const vector<Unidad>& catalogo) {
    int id;
    cout << "ID de unidad a agregar: "; id = errorDeCaracter();
    for (const auto& u : catalogo) {
        if (u.id == id) {
...
void modoDespliegue(vector<vector<string>>& map, vector<UnidadInst>& instancias) {
    ...
    cout << "Zona de despliegue permitida: Filas [" << (H_CAMPO - 5) << " a " << (H_CAMPO - 1) << "] y Columnas [0 a " << (W_CAMPO - 1) << "]\n";
    while (true) {
        cout << "\nInstancias:\n";
        ...
        if (op == 1) {
            ...
            cout << "Fila: "; fila = errorDeCaracter();
            cout << "Columna: "; col = errorDeCaracter();
...
void modoMovimiento(vector<vector<string>>& map, vector<UnidadInst>& instancias) {
    ...
    while (true) {
        ...
        if (op == 1) {
            ...
            cout << "Destino fila: "; fd = errorDeCaracter();
            cout << "Destino col: "; cd = errorDeCaracter();
...
string asignarArmasAUnidad(const vector<Arma>& armasDisponibles, int numTropas) {
    ...
    while (true) {
        cout << "id cantidad: ";
        idSel = errorDeCaracter();
        cntSel = errorDeCaracter();
...
void agregarArma(vector<Arma>& armas) {
    Arma a;
    cout << "ID del arma: "; a.id = errorDeCaracter();
    ...
    cout << "Impacto necesario (0 si impacto directo): "; a.impacto = errorDeCaracter();
    cout << "Penetracion: "; a.penetracion = errorDeCaracter();
    cout << "Dao por disparo (numero o D6): "; cin >> a.dano;
    cout << "Fuerza (opcional, 0 por defecto): "; a.fuerza = errorDeCaracter();
...
void editarArma(vector<Arma>& armas) {
    int id; cout << "ID del arma a editar: "; id = errorDeCaracter();
    for (auto& a : armas) {
        if (a.id == id) {
            ...
            cout << "Nuevo impacto (0 si directo): "; a.impacto = errorDeCaracter();
            cout << "Nueva penetracion: "; a.penetracion = errorDeCaracter();
            cout << "Nuevo dao: "; cin >> a.dano;
            cout << "Nueva fuerza: "; a.fuerza = errorDeCaracter();
            guardarArmas(armas);
...
void eliminarArma(vector<Arma>& armas) {
    int id; cout << "ID del arma a eliminar: "; id = errorDeCaracter();
...
void agregarArmaCuerpo(vector<ArmaCuerpo>& armasC) {
    ArmaCuerpo a;
    cout << "ID del arma cuerpo a cuerpo: "; a.id = errorDeCaracter();
    ...
    cout << "Fuerza (opcional, 0 por defecto): "; a.fuerza = errorDeCaracter();
    cout << "Golpes por asalto (opcional, 1 por defecto): "; a.golpes = errorDeCaracter();
...
void editarArmaCuerpo(vector<ArmaCuerpo>& armasC) {
    int id; cout << "ID del arma cuerpo a cuerpo a editar: "; id = errorDeCaracter();
    for (auto& a : armasC) {
        if (a.id == id) {
            ...
            cout << "Nuevo dao: "; cin >> a.dano;
            cout << "Nueva fuerza: "; a.fuerza = errorDeCaracter();
            cout << "Nuevos golpes por asalto: "; a.golpes = errorDeCaracter();
            guardarArmasCuerpo(armasC);
...
void eliminarArmaCuerpo(vector<ArmaCuerpo>& armasC) {
    int id; cout << "ID del arma cuerpo a cuerpo a eliminar: "; id = errorDeCaracter();
...
void agregarUnidad(vector<Unidad>& catalogo, const vector<Arma>& armasDisponibles, const vector<ArmaCuerpo>& armasCDisponibles) {
    ...
    cout << "Puntaje: "; u.puntaje = errorDeCaracter();
    cout << "Cantidad de tropas: "; u.numTropas = errorDeCaracter();
    cout << "Movimiento: "; u.movimiento = errorDeCaracter();
    cout << "Salvacion: "; u.salvacion = errorDeCaracter();
    cout << "Heridas: "; cin >> u.heridas;
    cout << "Impacto: "; u.impacto = errorDeCaracter();
...
void editarUnidad(vector<Unidad>& catalogo, const vector<Arma>& armasDisponibles, const vector<ArmaCuerpo>& armasCDisponibles) {
    ...
    for (auto& u : catalogo) {
        if (u.id == id) {
            ...
            cout << "Nuevo puntaje: "; u.puntaje = errorDeCaracter();
            cout << "Nueva cantidad de tropas: "; u.numTropas = errorDeCaracter();
            cout << "Nuevo movimiento: "; u.movimiento = errorDeCaracter();
            cout << "Nuevo salvacion: "; u.salvacion = errorDeCaracter();
            cout << "Nuevas heridas: "; cin >> u.heridas;
            cout << "Nuevo impacto: "; u.impacto = errorDeCaracter();

            cout << "Asignacion de armas a distancia actual: " << (u.armas.empty() ? "-" : u.armas) << "\n";
            cout << "Deseas re-asignar armas a distancia? 1=Si 0=No: ";
            int ra; ra = errorDeCaracter();
            if (ra == 1) {
                u.armas = asignarArmasAUnidad(armasDisponibles, u.numTropas);
            }
...
void faseCuerpoACuerpo(
    std::vector<std::vector<string>>& mapa,
    std::vector<UnidadInst>& instancias,
    const std::vector<Unidad>& catalogo,
    const std::vector<ArmaCuerpo>& armasCDisponibles,
    bool jugadorTurno
...
int main() {
    ...
    while (true) {
        cout << "=== SISTEMA DE LOGIN / REGISTRO ===\n"
             ...
             << "3) Salir\nElige: ";
        int preop; preop = errorDeCaracter();
        ...
        while (true) {
            if (actual.tipo == 1) {
                cout << "\n===== MENU JUGADOR =====\n"
                     << "1. Jugar Batalla (IA vs Jugador)\n"
                     << "2. Gestionar mi Ejercito Personal\n"
                     << "3. Consultar Catalogo General\n"
                     << "4. Cerrar Sesion\n";
                cout << "Opcion: "; op = errorDeCaracter();
                if (op == 1) {
                    ...
                    if (puntosJugador <= 0) {
                        ...
                        if (ejercitoIA.empty()) {
                            ...
                            modoBatalla(mapa, instancias, catalogo, armas, armasC);
                            for (const auto &ui : instanciasIA) quitarUnidadDelMapa(mapa, ui);
                    ...
                    cout << "\n--- GESTION DE EJERCITO ---\n"
                         << "1. Ver mi ejercito\n"
                         << "2. Agregar unidad al ejercito\n"
                         << "3. Eliminar unidad del ejercito\n"
                         << "4. Volver\nOpcion: "; op2 = errorDeCaracter();
                    if (op2 == 1) mostrarEjercito(ejercito, catalogo);
                    else if (op2 == 2) { agregarAlEjercito(actual.username, ejercito, catalogo); instancias = generarInstanciasDesdeEjercito(ejercito, catalogo); }
                    else if (op2 == 3) { eliminarDelEjercito(actual.username, ejercito); instancias = generarInstanciasDesdeEjercito(ejercito, catalogo); }
                }
                else if (op == 3) mostrarCatalogo(catalogo);
                else if (op == 4) break;
            }
            else if (actual.tipo == 2) {
                cout << "\n===== MENU EDITOR =====\n"
                     << "1. Gestionar Unidades del Catalogo\n"
                     << "2. Gestionar Armería (Distancia / Cuerpo a Cuerpo)\n"
                     << "3. Mi Ejercito y Mapa\n"
                     << "4. Cerrar Sesion\n";
                cout << "Opcion: "; op = errorDeCaracter();
                if (op == 1) {
                    int op2;
                    cout << "\n--- GESTION DE CATALOGO ---\n"
                         ...
                         << "3. Editar unidad del catalogo\n"
                         << "4. Eliminar unidad del catalogo\n"
                         << "5. Volver\nOpcion: "; op2 = errorDeCaracter();
                    if (op2 == 1) mostrarCatalogo(catalogo);
                    else if (op2 == 2) agregarUnidad(catalogo, armas, armasC);
                    else if (op2 == 3) editarUnidad(catalogo, armas, armasC);
                    else if (op2 == 4) eliminarUnidad(catalogo);
                }
                else if (op == 2) {
                    int op2;
                    cout << "\n--- GESTION DE ARMERIA ---\n"
                         << "1. Ver armas a distancia\n"
                         << "2. Agregar arma a distancia\n"
                         << "3. Editar arma a distancia\n"
                         << "4. Eliminar arma a distancia\n"
                         << "5. Ver armas cuerpo a cuerpo\n"
                         << "6. Agregar arma cuerpo a cuerpo\n"
                         << "7. Editar arma cuerpo a cuerpo\n"
                         << "8. Eliminar arma cuerpo a cuerpo\n"
                         << "9. Volver\nOpcion: "; op2 = errorDeCaracter();
                    if (op2 == 1) mostrarArmas(armas);
                    else if (op2 == 2) agregarArma(armas);
                    else if (op2 == 3) editarArma(armas);
                    else if (op2 == 4) eliminarArma(armas);
                    else if (op2 == 5) mostrarArmasCuerpo(armasC);
                    else if (op2 == 6) agregarArmaCuerpo(armasC);
                    else if (op2 == 7) editarArmaCuerpo(armasC);
                    else if (op2 == 8) eliminarArmaCuerpo(armasC);
                }
                else if (op == 3) {
                    int op2;
                    cout << "\n--- EJERCITO Y MAPA ---\n"
                         << "1. Mostrar mapa actual\n"
                         << "2. Ver mi ejercito personal\n"
                         << "3. Agregar unidad a mi ejercito\n"
                         << "4. Volver\nOpcion: "; op2 = errorDeCaracter();
                    if (op2 == 1) mostrarMapa(mapa);
                    else if (op2 == 2) mostrarEjercito(ejercito, catalogo);
                    else if (op2 == 3) { agregarAlEjercito(actual.username, ejercito, catalogo); instancias = generarInstanciasDesdeEjercito(ejercito, catalogo); }
                }
                else if (op == 4) break;
                ...
                cout << "\n===== MENU ADMINISTRADOR =====\n"
                     ...
                     << "2. Gestionar Catalogo de Unidades\n"
                     << "3. Gestionar Armeria\n"
                     << "4. Ver Mapa y Mi Ejercito\n"
                     << "5. Cerrar Sesion\n";
                cout << "Opcion: "; op = errorDeCaracter();
                if (op == 1) {
                    int op2;
                    cout << "\n--- GESTION DE USUARIOS ---\n"
                         ...
                         << "5. Volver\nOpcion: "; op2 = errorDeCaracter();
                    if (op2 == 1) listarUsuarios(usuarios);
                    else if (op2 == 2) registrarUsuarioPublico(usuarios);
                    else if (op2 == 3) {
                        cout << "Usuario a eliminar: "; string udel; cin >> udel;
                        ...
                        cout << "Usuario a modificar: "; string umod; cin >> umod;
                        int idx = buscarUsuario(usuarios, umod);
                        if (idx != -1) {
                            cout << "Nuevo tipo (1,2,3): "; int nt; nt = errorDeCaracter();
                    ...
                    cout << "\n--- GESTION DE ARMERIA ---\n"
                         << "1. Ver armas a distancia\n"
                         << "2. Agregar/Editar/Eliminar armas distancia\n"
                         << "3. Ver armas cuerpo a cuerpo\n"
                         << "4. Agregar/Editar/Eliminar armas cuerpo a cuerpo\n"
                         << "5. Volver\nOpcion: "; op2 = errorDeCaracter();
                    if (op2 == 1) mostrarArmas(armas);
                    else if (op2 == 2) {
                        int op3; cout << "1.Agregar 2.Editar 3.Eliminar: "; op3 = errorDeCaracter();
                        if (op3 == 1) agregarArma(armas); else if (op3 == 2) editarArma(armas); else if (op3 == 3) eliminarArma(armas);
                    }
                    else if (op2 == 3) mostrarArmasCuerpo(armasC);
                    else if (op2 == 4) {
                        int op3; cout << "1.Agregar 2.Editar 3.Eliminar: "; op3 = errorDeCaracter();
                        if (op3 == 1) agregarArmaCuerpo(armasC); else if (op3 == 2) editarArmaCuerpo(armasC); else if (op3 == 3) eliminarArmaCuerpo(armasC);
                    }
                }
                else if (op == 4) {
                    int op2;
